...
void invmixcolumn(uint8_t s[4][4])
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        s[0][i] = multi(0x0e, a) ^ multi(0x0b, b) ^ multi(0x0d, c) ^ multi(0x09, d);
        s[1][i] = multi(0x09, a) ^ multi(0x0e, b) ^ multi(0x0b, c) ^ multi(0x0d, d);
        s[2][i] = multi(0x0d, a) ^ multi(0x09, b) ^ multi(0x0e, c) ^ multi(0x0b, d);
        s[3][i] = multi(0x0b, a) ^ multi(0x0d, b) ^ multi(0x09, c) ^ multi(0x0e, d);
    }
}

// Function to add round key
void Add_roundkey(uint8_t state[4][4], uint8_t roundkey[176], int round)
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
...
void invsub_byte(uint8_t state[4][4])
{
    for (int i = 0; i < 4; i++)
    {
        for (int j = 0; j < 4; j++)
